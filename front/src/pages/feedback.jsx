...
import "./feedback.css"; // Removido para corrigir o erro de build
...
export default function Feedback() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!data.titulo || !data.descricao || !data.tipoUsuario || !data.categoria) {
      // Usar um método de notificação melhor que alert() no futuro
      console.error("Preencha todos os campos");
      return;
    }

    // Adiciona o novo feedback no início da lista
  ...
  const removerFeedback = (index) => {
    // Filtra o feedback a ser removido
              ...
              {meusFeedbacks.map((fb, i) => (
                <article key={i} className={`feedback-card ${catToClass[fb.categoria] || "azul"}`}>
                  <div className="card-header">
                    <h3>{fb.titulo}</h3>
                    <button className="btn-excluir" onClick={() => removerFeedback(i)}>Excluir</button>
                  </div>
                  <small><strong>Enviado por:</strong> {fb.tipoUsuario}</small>
                  <p>{fb.descricao}</p>
                  <div className="card-footer">
                    <small><strong>Categoria:</strong> {fb.categoria}</small>
                  </div>
                </article>
              ))}
            </div>
